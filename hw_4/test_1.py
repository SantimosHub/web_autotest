...
import logging
import os
from testpage import OperationsHelper, ids
...
def test_successful_login(browser):
    ...
    test_page.click_login_button()
    hello_text = test_page.find_element(ids["LOCATOR_HELLO_TEXT"]).text
...
def test_user_can_create_post(browser):
    ...
    test_page.save_post()
    time.sleep(2)
    post_title = test_page.find_element(ids["LOCATOR_POST_TITLE"]).text
    logging.info(f"We found text {post_title} on the page of the post")
    assert post_title == testdata['title'], "The post wasn't created"
...
def test_check_contact_us(browser):
    ...
    contact_page = OperationsHelper(browser)
